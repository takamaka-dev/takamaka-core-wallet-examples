 ...
 */
@Slf4j
public class SubmitRegisterMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        log.info(" --- same code TransactionRegisterMainED25519 --- Begin ---");
        ...
        InternalTransactionBean registerMainITB = BuilderITB.registerMain(
                publicKeySource,
                "test register main",
        ...
        TransactionBean myRegisterMainObject
        ...
        String registerMainTransactionJson = TkmTextUtils.toJson(
                myRegisterMainObject);

        log.info("the serialized transaction");
        log.info(registerMainTransactionJson);

        log.info("How to perform a syntactic check of the newly created "
                + "transaction.");

        TransactionBox registerMainTbox = TkmWallet.verifyTransactionIntegrity(
                registerMainTransactionJson);
        log.info("the transaction is valid?: " + registerMainTbox.isValid());

        FeeBean registerMainFeeBean = TransactionFeeCalculator.getFeeBean(
                registerMainTbox);

        log.info("REGISTER MAIN is a basic transaction");
        log.info("single inclusion transaction hash: " + registerMainFeeBean.getSith()
                + "\nCPU cost (nanoTK):\t" + registerMainFeeBean.getCpu()
                + "\nMEMORY cost (nanoTK):\t" + registerMainFeeBean.getMemory()
                + "\nDISK cost (nanoTK):\t" + registerMainFeeBean.getDisk()
        );
        log.info("readable way in TK: " + TransactionFeeCalculator.getCostInTK(
                registerMainFeeBean).toPlainString());

        log.info(" --- same code TransactionRegisterMainED25519 --- End ---");
        ...
        String registerMainHexBody = TkmSignUtils.fromStringToHexString(
                registerMainTransactionJson);

        log.info("the wrapped json, can be decode using hex to text tool");
        log.info(registerMainHexBody);
        ...
        String registerMainTxVerifyResult = ProjectHelper.doPost(
        ...
        log.info(registerMainTxVerifyResult);
